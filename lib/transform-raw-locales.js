...
module.exports = function start (transformConfig) {
  ...
  const fallbackTranslation = readJSON(path.resolve(fullTranslationDir, `${transformConfig.raw.fallbackLang}.json`))
  ...
  files.forEach(filename => {
    ...
    // 默认文本不做任何处理
    if (!filename.toLowerCase().includes(transformConfig.defaultLang)) {
      // 对于繁体中文特殊处理下
      if (filename.match(/-hant|-tw|-hk/gi) && transformConfig.autoS2T) {
        transformedTranslation = zhCN2zhTW(translation)
      }
      // 是否使用默认语言代替未翻译文本
      else if (transformConfig.raw.fallbackLang) {
